...
from . import lightdgcnn_modeling
...
class TaskConfigs(object):
	...
	def __init__(self, configs):
		...
		self.predict_checkpoint = configs.get("predict_checkpoint", None)    
		self.use_layer_lr = configs.get("use_layer_lr", False)
		self.whole_or_feature = configs.get("whole_or_feature", 'whole')
...
def create_model(
	model_type, bert_config, is_training, input_ids, input_mask, segment_ids, labels, num_labels, 
	use_one_hot_embeddings, num_train_steps):
	"""Create a classification model."""
	use_one_hot_embeddings = True
	if model_type not in ['bert_struct', 'bert_dynamic']:
	...
	if model_type.startswith("bert") or model_type.startswith("roberta"):
	...
	elif model_type.startswith("electra_v1"):
		model = modeling.BertModel(
			config=bert_config,
			is_training=is_training,
			input_ids=input_ids,
	...
	elif model_type.startswith("electra_official"):
		...
		model = lightdgcnn_modeling.LightDGCNN(
			bert_config,
			 is_training,
			 input_ids, 
			 reuse=None,
	...
	output_weights = tf.get_variable(
		"task_specific/output_weights", [num_labels, hidden_size],
		initializer=tf.truncated_normal_initializer(stddev=0.02))
	
	output_bias = tf.get_variable(
		"task_specific/output_bias", [num_labels], 
	...
	##############bi_tempered_logistic_loss#############################################################################
		...
		gp_type = "none"
		if gp_type == "normal":
			gp = tf.reduce_sum(tf.gradients(loss, [embedding_matrix])[0]**2)
			gp_loss = 0.5 * gp
		elif gp_type == "normalized":
			gp = tf.sqrt(tf.reduce_sum(tf.gradients(loss, [embedding_matrix])[0]**2))
			gp_loss = 1.0 * gp
		else:
...
def model_fn_builder(model_type, bert_config, num_labels, init_checkpoint, learning_rate,
			...
			use_layer_lr, whole_or_feature='whole'):
	"""Returns `model_fn` closure for TPUEstimator."""
	def model_fn(features, labels, mode, params): # pylint: disable=unused-argument
		"""The `model_fn` for TPUEstimator."""
		
		tf.logging.info("*** Features ***")
		tf.logging.info("*** model type: %s ***"%(model_type))
		...
		total_loss, per_example_loss, logits, probabilities = create_model(
			model_type, bert_config, is_training, input_ids, input_mask, segment_ids, label_ids, num_labels, 
			use_one_hot_embeddings, num_train_steps)
		...
		scaffold_fn = None
		if init_checkpoint:
			if model_type.startswith("light_dgcnn") or model_type.startswith("bert") or model_type.startswith("roberta") or model_type.startswith("electra"):
				assignment_map, initialized_variable_names = modeling.get_assignment_map_from_checkpoint(tvars, init_checkpoint)
			else:
				assignment_map, initialized_variable_names = albert_modeling.get_assignment_map_from_checkpoint(tvars, init_checkpoint)
		if use_tpu:
			def tpu_scaffold():
...
def run_classifier(processor, configs):
	...
	if configs.do_train:
		...
		save_checkpoints_steps = int(len(train_examples) / configs.train_batch_size)
	...
	model_fn = model_fn_builder(
		...
		use_layer_lr=configs.use_layer_lr,
		whole_or_feature=configs.whole_or_feature)
		...
		with tf.gfile.GFile(output_eval_file, "w") as writer:
			for global_step, filename in sorted(steps_and_files, key=lambda x: x[0]):
				...
				if result['eval_accuracy'] > best_accuracy:
					...
					tf.logging.info("***** Eval results %s:%s *****" % (filename, str(result['eval_accuracy'])))
	...
	if configs.do_predict:
		...
		predict_input_fn = file_based_input_fn_builder(
			...
			drop_remainder=predict_drop_remainder,
                        batch_size=1)
		if configs.predict_checkpoint:
			filename = configs.predict_checkpoint
			tf.logging.info("***** Running prediction checkpoint %s*****"%(filename))
