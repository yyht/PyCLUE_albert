...
def create_model(
    ...
    elif model_type.startswith("bert_ebm"):
        model = modeling.BertModel(
            config=bert_config,
            is_training=is_training,
            input_ids=input_ids,
    ...
    elif model_type.startswith("bert_ebm_gen"):
        model = modeling.BertModel(
            config=bert_config,
            is_training=is_training,
            input_ids=input_ids,
...
def model_fn_builder(model_type, bert_config, num_labels, init_checkpoint, learning_rate,
    ...
    def model_fn(features, labels, mode, params): # pylint: disable=unused-argument
        ...
        scaffold_fn = None
        if init_checkpoint:
            if model_type.startswith("bert") or model_type.startswith("roberta") or model_type.startswith("electra"):
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            train_op = optimization.create_optimizer(
                total_loss, learning_rate, num_train_steps, num_warmup_steps, use_tpu,
		              layerwise_lr_decay_power=0.8, n_transformer_layers=bert_config.num_hidden_layers,
                      task_name="task_specific/",
                      whole_or_feature=whole_or_feature)
